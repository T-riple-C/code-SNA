...
for model in predModel:
    res = pd.DataFrame({'fname': ["fname"], 'roc_auc': ["roc_auc"], 'acc': ["acc"], 'mcc': ["mcc"], 'type': ['metrics']})
    res.to_csv("../../res/rq1_2-res/within-project/pred_res/"+model+"/predRes.csv", mode='a', sep=',', index=False, header=False)
    for label in ['all_code','code_nosize','cs_']:
        for i in range(len(dataset)):
            ...
            for train_index, test_index in kf.split(X):
                ...
                # 1.4 balance
                if (np.sum(y_train == 1) <= np.sum(y_train == 0) / 2):
            ...
            tmp.append(label)
            res.loc[0] = tmp
            res.to_csv("../../res/rq1_2-res/within-project/pred_res/"+model+"/predRes.csv", mode='a', sep=',', index=None, header=False)
    print(model+" done.")
    # break
